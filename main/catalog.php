...
<div class="container">
    ...
    <div class="pagination">
        <?php if ($page > 1): ?>
            <a class="button" href="?page=1&type=<?= $id_type ?>&min=<?= $min ?>&max=<?= $max ?>&sort=<?= $sort ?>">Первая</a>
            <a class="button" href="?page=<?= $page - 1 ?>&type=<?= $id_type ?>&min=<?= $min ?>&max=<?= $max ?>&sort=<?= $sort ?>">Назад</a>
        <?php endif; ?>
        <span>Страница <?= $page ?> из <?= $total_pages ?></span>
        <?php if ($page < $total_pages): ?>
            <a class="button" href="?page=<?= $page + 1 ?>&type=<?= $id_type ?>&min=<?= $min ?>&max=<?= $max ?>&sort=<?= $sort ?>">Вперёд</a>
            <a class="button" href="?page=<?= $total_pages ?> &type=<?= $id_type ?>&min=<?= $min ?>&max=<?= $max ?>&sort=<?= $sort ?>">Последняя</a>
        <?php endif; ?>
    </div>
</div>
</div>
