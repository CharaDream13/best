...
$stmt = $connect->prepare("
    SELECT r.comment, r.date, r.rating, r.show_first_name, r.show_last_name, r.show_patronymic, r.id_user, r.show_login, u.first_name, u.last_name, u.patronymic, u.login
    FROM Reviews r LEFT JOIN Users u ON r.id_user = u.id_user WHERE r.id_moder_status = 2 ORDER BY RAND() LIMIT 10");
$stmt->execute();
$connect->set_charset('utf8');
$stmt->bind_result($comment, $date, $rating, $show_first_name, $show_last_name, $show_patronymic, $id_user, $show_login, $first_name, $last_name, $patronymic, $login);
$reviews = array();
while ($stmt->fetch()) {
    $reviews[] = array(
        'comment' => $comment,
    ...
    <div class="container">
        ...
        <div class="container_grid">
            ...
            <div>
                ...
                <p>
                    Готовы предоставить смежные услуги например, вывоз мусора, вывоз грунта, уборка снега или аренда
                    самосвала.
                </p>
            </div>
        </div>
    ...
    <div class="container">
        ...
        <div class="container_flex">
            <div>
                ...
                <p>За короткий срок мы достигли определенных высот на рынке став надежными партнерами для наших
                    клиентов.</p>
                <p>Лучше любых слов о нас скажут факты и цифры:</p>
                <p>1 Наша техника задействована в строительстве «Северный поток-2».Этот строящийся магистральный
                    соединяющийся газопровод из России в Германию. Его протяженность составляет более 1200 км;</p>
                <p>2 Участвуем в реконструкции автомобильной дороги федерального значения М4-Дон. Протяжённость
                    автодороги составляет 1543 км;</p>
                <p>3 Наши специалисты участвуют в строительстве и реконструкции газораспределительных станций на
                    территории Ленинградской области;</p>
                <p>4 Обеспечиваем техникой строящиеся жилищные комплексы на территории всего Северо-Западного
                    Федерального района,</p>
